...
void robot::setTargetXY(double x_target, double y_target)
{
    ...
    int map_x = std::round(x_target * (MAP_WIDTH / 5.21) + 50);
    int map_y = std::round(y_target * (MAP_HEIGHT / 6.02) + 470);
    float temp_x[50] = {0}; 
    float temp_y[50] = {0};
    int temp_num = 0;
...
void robot::startFindYourself()
{
    std::cout << "Starting find yourself sequence..." << std::endl;
    
    {
        ...
        x_target_position[0] = 0.5;  y_target_position[0] = 0.5;
        x_target_position[1] = 0.5;  y_target_position[1] = -0.5;
        x_target_position[2] = -0.5; y_target_position[2] = -0.5;
        x_target_position[3] = -0.5; y_target_position[3] = 0.5;
    }
    
    mc.startFindYourselfThread();
}
...
void robot::updateArcTrajectory()
{
    ...
    const bool obstacle_now = (!mc.foundMyself) ? true : obstacleDetector(distance_to_target, angle_to_target);
...
void robot::updateLidarSegments()
{
    ...
    const double safety_radius_m = (wheel_base_distance * 1.2) + (0.1 * speed_m_per_s);
    ...
    std::vector<double> segment_minimal_values(segment_count, -1.0);
    ...
    for (int seg = 0; seg < segment_count; seg++)
    {
        ...
        for (size_t i = start; i < end; i++)
        {
            ...
            if (scan_mm <= 0.0)
                continue;
            if (valid_count == 0 || scan_mm < min_mm)
                min_mm = scan_mm;
            valid_count++;
        }
        ...
        segment_minimal_values[seg] = min_mm;
 
        if (min_mm > this->threshold_mm)
            next_segment_state[seg] = 0;
        else if (min_mm > this->hysteresis_mm)
    ...
    for (int seg = 0; seg < segment_count; seg++)
    {
        if (next_segment_state[seg] < 1)
            continue;
        if (segment_minimal_values[seg] <= 0.0)
            continue;

        const double distance_m = segment_minimal_values[seg] * 0.001;
        if (distance_m <= 0.0)
            continue;
